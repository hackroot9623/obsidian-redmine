...
__export(main_exports, {
  ...
  KANBAN_VIEW_TYPE: () => KANBAN_VIEW_TYPE2,
...
var RedmineClient = class {
  ...
  async getIssueAllowedStatuses(issueId) {
  ...
  async searchIssues(query, projectId, userId, sprintId) {
    const limit = 100;
    let offset = 0;
    let allIssues = [];
    let hasMore = true;
...
var import_dragula = __toESM(require("dragula"));
var KANBAN_VIEW_TYPE = "redmine-kanban-view";
var KanbanView = class extends import_obsidian7.ItemView {
  constructor(leaf, redmineClient) {
    super(leaf);
    this.issues = [];
    this.statuses = [];
    this.projects = [];
  ...
  async onOpen() {
    ...
    this.intervalId = window.setInterval(() => this.refresh(), 5 * 60 * 1e3);
  }
  async loadStatuses() {
    try {
      ...
      new import_obsidian7.Notice(`Error loading issue statuses: ${e.message}`);
    }
  }
  async loadProjects(selector) {
    try {
      const response = await this.redmineClient.getProjects();
      this.projects = response.projects;
      this.projects.forEach((project) => {
      ...
      console.error("Error loading projects:", e);
      new import_obsidian7.Notice(`Error loading projects: ${e.message}`);
    }
  }
  async loadIssues() {
    if (!this.selectedProjectId) {
    ...
    try {
      ...
      if (activeSprint) {
        const response = await this.redmineClient.searchIssues("", this.selectedProjectId, void 0, activeSprint.id);
      ...
      new import_obsidian7.Notice(`Error loading issues: ${e.message}`);
    }
  }
  renderKanbanBoard() {
    const container = this.containerEl.children[1];
    ...
    this.drake = (0, import_dragula.default)(columns);
    this.drake.on("drop", async (el, target, source, sibling) => {
      ...
      try {
        ...
        if (isAllowed) {
          ...
          new import_obsidian7.Notice("Issue status updated successfully.");
        } else {
          new import_obsidian7.Notice("Invalid status transition.", "error");
          source.appendChild(el);
        }
      } catch (e) {
        console.error("Error updating issue status:", e);
        new import_obsidian7.Notice(`Error updating issue status: ${e.message}`);
        source.appendChild(el);
      }
    });
  }
...
var KANBAN_VIEW_TYPE2 = "redmine-kanban-view";
...
var RedminePlugin = class extends import_obsidian8.Plugin {
  async onload() {
    ...
    this.addCommand({
      ...
      callback: async () => {
        this.app.workspace.detachLeavesOfType(KANBAN_VIEW_TYPE2);
        await this.app.workspace.getRightLeaf(false).setViewState({
          type: KANBAN_VIEW_TYPE2,
          active: true
        });
        this.app.workspace.revealLeaf(
          this.app.workspace.getLeavesOfType(KANBAN_VIEW_TYPE2)[0]
        );
      }
    });
    this.addCommand({
...
var RedmineSettingTab = class extends import_obsidian8.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian8.Setting(containerEl).setName("Redmine URL").setDesc("The base URL of your Redmine instance (e.g., https://your-redmine.com).").addText((text) => text.setPlaceholder("https://redmine.example.com").setValue(redmineUrl).onChange((value) => {
      redmineUrl = value;
    }));
    new import_obsidian8.Setting(containerEl).setName("Redmine API Key").setDesc("Your Redmine API access key. You can find this in your Redmine profile settings.").addText((text) => text.setPlaceholder("Enter your API key").setValue(redmineApiKey).onChange((value) => {
      redmineApiKey = value;
    }));
    new import_obsidian8.Setting(containerEl).addButton((button) => button.setButtonText("Test Connection").onClick(async () => {
    ...
    new import_obsidian8.Setting(containerEl).setName("Your Redmine User").setDesc('Select your Redmine user from the dropdown. This is used for "Created by" field in new issues.').addDropdown(async (dropdown) => {
      try {
        ...
        new import_obsidian8.Notice(`Failed to load Redmine users: ${e.message}`);
        console.error(e);
      }
    });
    containerEl.createEl("h2", { text: "AI Settings" });
    new import_obsidian8.Setting(containerEl).setName("Gemini API Key").setDesc("Your Google Gemini API key. This is required for AI-powered description generation.").addText((text) => text.setPlaceholder("Enter your API key").setValue(geminiApiKey).onChange((value) => {
      geminiApiKey = value;
    }));
    new import_obsidian8.Setting(containerEl).setName("AI Description Language").setDesc(`The language for AI-generated descriptions (e.g., 'en' for English, 'es' for Spanish, 'fr' for French).`).addText((text) => text.setPlaceholder("en").setValue(language).onChange((value) => {
      language = value;
    }));
    new import_obsidian8.Setting(containerEl).setName("Default View").setDesc("Select the default view to open when clicking the ribbon icon.").addDropdown((dropdown) => {
      dropdown.addOption(ISSUES_VIEW_TYPE, "Issues View");
      dropdown.addOption(TASK_LIST_VIEW_TYPE, "Task List View");
      dropdown.addOption(KANBAN_VIEW_TYPE2, "Kanban View");
      dropdown.setValue(defaultView);
      dropdown.onChange((value) => {
        defaultView = value;
      });
    });
    new import_obsidian8.Setting(containerEl).addButton((button) => button.setButtonText("Save Settings").setCta().onClick(async () => {
      ...
      new import_obsidian8.Notice("Settings saved successfully!");
    }));
  }
};
